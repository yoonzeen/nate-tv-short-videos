...
- **Node.js**: 20.9.0 이상 (권장: 22 LTS)
- **메모리**: 최소 512MB
- **포트**: 3000번 (기본값, 변경 가능)
- **네트워크**: 외부 API 호출 가능 (네이트 뉴스 크롤링)
...
### 네이트 뉴스 피드 (`/news`)
- 실시간 네이트 뉴스 관심 랭킹 1-100위 크롤링
- 10개마다 광고 삽입 (총 110개 아이템)
- 각 기사별 추천수, 베스트 댓글 표시
- 소셜 공유 기능 (동적 OG 태그)
- 자동 슬라이드 (15초 간격)

### IssuePlus 피드 (`/issueplus`)
- 정적 이슈플러스 콘텐츠
- 세로형 스냅 스크롤
- 영상 자동 재생
